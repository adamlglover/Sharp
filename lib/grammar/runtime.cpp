...
void runtime::parseStatement(Block& block, ast* pAst) {
    ...
    if(!current_scope()->reachable) {
        ...
        current_scope()->last_statement=pAst->gettype();
    }
...
void runtime::parseConstructorDecl(ast* pAst) {
    ...
    if(method != NULL) {
        ...
        fblock.code.push_i64(SET_Ci(i64, op_MOVR, adx,0, fp));
...
void runtime::pushAuthenticExpressionToStackNoInject(Expression& expression, Expression& out) {
    ...
    switch(expression.type) {
        ...
        case expression_field:
            if(expression.utype.field->nativeInt() && !expression.utype.field->array) {
                ...
                out.code.push_i64(SET_Di(i64, op_SMOVOBJ, 0));
            } else if(expression.utype.field->dynamicObject() || expression.utype.field->type == field_class) {
                out.code.push_i64(SET_Di(i64, op_INC, sp));
                out.code.push_i64(SET_Ci(i64, op_MOVR, adx,0, sp));
                out.code.push_i64(SET_Di(i64, op_SMOVOBJ, 0));
...
void runtime::constructNewString(Expression &stringExpr, Expression& out) {
    List<Expression> expressions;
    List<Param> params;
    ClassObject* klass;
    Method* fn=NULL;
...
bool runtime::constructNewString(Expression &stringExpr, Expression &right, token_entity operand, Expression& out, ast* pAst) {
    List<Expression> expressions;
    List<Param> params;
    ClassObject* klass;
    Method* fn=NULL;
    ...
    if((klass = getClass("std", "string")) != NULL) {
        if((fn=klass->getConstructor(params)) != NULL) {
            ...
            stringExpr.utype.type=ResolvedReference::CLASS;
            out.type = expression_lclass;
            out.utype.klass = klass;
            out.utype.type=ResolvedReference::CLASS;
...
Expression runtime::parseSizeOfExpression(ast* pAst) {
    ...
    switch(expression.type) {
        ...
        case expression_null:
        case expression_string:
            errors->newerror(GENERIC, expression.lnk, "cannot get sizeof from expression of type `" + expression.typeToString() + "`");
            break;
        default:
            if(expression.type == expression_var && expression.literal) {
                errors->newerror(GENERIC, expression.lnk, "cannot get sizeof from literal value");
            }
...
Expression &runtime::parseDotNotationChain(ast *pAst, Expression &expression, unsigned int startpos) {
    ...
    for(unsigned int i = startpos; i < pAst->getsubastcount(); i++) {
            ...
            if(utype->gettype() == ast_dotnotation_call_expr) {
                ...
                if(rightExpr.func && rightExpr.type != expression_void) {
                    expression.code.__asm64.insert(0, SET_Di(i64, op_INC, sp));
                    expression.code.__asm64.insert(1, SET_Ei(i64, op_INIT_FRAME));
                } else if(rightExpr.func) {
                    expression.code.__asm64.insert(1, SET_Ei(i64, op_INIT_FRAME));
                }
            }
...
void runtime::addStringConstruct(token_entity operand, ClassObject* klass, Expression& out, Expression& left, Expression &right, ast* pAst) {
    List<Param> params;
    List<Expression> eList;
    eList.push_back(right);
    OperatorOverload* overload;
    ...
    if((overload = klass->getOverload(string_toop(operand.gettoken()), params)) != NULL) {
        // call operand
        if(overload->type != lvoid)
            newOut.code.push_i64(SET_Di(i64, op_INC, sp));

        newOut.code.push_i64(SET_Ei(i64, op_INIT_FRAME));
        ...
        if(out.type == expression_lclass) {
            ...
            left.utype.type = ResolvedReference::CLASS;
        }

        out.func=true;
        newOut.code.push_i64(SET_Di(i64, op_CALL, overload->vaddr));
...
void runtime::addClassChain(token_entity operand, ClassObject* klass, Expression& out, Expression& left, Expression &right, ast* pAst) {
    List<Param> params;
    List<Expression> eList;
    eList.push_back(right);
    OperatorOverload* overload;
    ...
    if((overload = klass->getOverload(string_toop(operand.gettoken()), params)) != NULL) {
        ...
        if(left.func && left.type != expression_void) {
            ...
            if(overload->type != lvoid)
                out.code.push_i64(SET_Di(i64, op_INC, sp)); // we dont need to increment bcause we just use the slot of th old objects location
        ...
        if(left.arrayElement) {
            ...
            for(unsigned int i = 0; i < left.code.size(); i++)
            {
                if(GET_OP(left.code.__asm64.get(i)) == op_MOVX) {
                    ...
                    left.code.__asm64.insert(--i, SET_Ci(i64, op_MOVR, adx,0, sp));
                    left.code.__asm64.insert(i+1, SET_Ci(i64, op_SMOV, egx,0, -1));
                    left.code.__asm64.insert(pos+2, SET_Ci(i64, op_RMOV, ebx,0, egx));
                    break;
                }
            }
            out.inject(left);
